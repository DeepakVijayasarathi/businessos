...
import { Plus, Search, BookOpen, Eye, ThumbsUp, ThumbsDown, ChevronRight, X, Edit, Trash2, Sparkles, Loader2 } from 'lucide-react';
...
function ArticleModal({ article, categories, onClose }: { article: any; categories: any[]; onClose: () => void }) {
  ...
  const [showAiBanner, setShowAiBanner] = useState(!article);

  async function generateWithAI() {
    const topic = aiTopic || form.title;
    if (!topic) return toast.error('Enter a topic or title first');
    setAiLoading(true);
    try {
      const { data } = await api.post('/ai/kb-article', { topic, audience: 'customers and support staff', tone: 'professional and helpful' });
      setForm(f => ({ ...f, title: data.data.title || f.title, content: data.data.content || '', excerpt: data.data.summary || f.excerpt, tags: (data.data.tags || []).join(', ') || f.tags }));
          ...
          {showAiBanner && (
            <div className="rounded-xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/30 p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-indigo-500 mb-2">Enter a topic and AI will write the full article</p>
                <div className="flex gap-2">
                  <input
                    value={aiTopic}
                    onChange={e => setAiTopic(e.target.value)}
                    placeholder="e.g. How to reset your password"
                    className="flex-1 text-sm rounded-lg px-3 py-1.5 border border-indigo-200 dark:border-indigo-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-400"
                    onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); generateWithAI(); } }}
                  />
                  <button type="button" onClick={generateWithAI} disabled={aiLoading} className="px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                    {aiLoading ? 'Generating…' : 'Generate'}
                  </button>
                  <button type="button" onClick={() => setShowAiBanner(false)} className="text-indigo-400 hover:text-indigo-600 px-1"><X className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          )}
          {!showAiBanner && !article && (
            <button type="button" onClick={() => setShowAiBanner(true)} className="flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
