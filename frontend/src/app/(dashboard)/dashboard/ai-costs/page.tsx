...
import {
  DollarSign, Zap, BarChart3, Activity, TrendingUp, TrendingDown,
  RefreshCw, ChevronDown, Brain, MessageSquare, FileText, BookOpen,
  Target, Users, Layers,
} from 'lucide-react';
...
export default function AICostsPage() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<UsageStats>({
  ...
  // Daily chart — last 14 days max for readability
  ...
  (data?.byModel || []).forEach(m => {
    providerTotals[m.provider] = (providerTotals[m.provider] || 0) + m.costUsd;
  });
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200 dark:border-gray-700/60 p-4 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {fmt(data?.totals.inputTokens || 0)} in · {fmt(data?.totals.outputTokens || 0)} out
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200 dark:border-gray-700/60 p-5 shadow-sm">
              ...
              {data?.byModel && data.byModel.length > 0 ? (
                      ...
                      <div key={i}>
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-700 dark:text-gray-300 font-medium truncate max-w-[140px]">{m.model}</span>
                            <span className="text-[10px] text-gray-400 uppercase">{m.provider}</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-[11px] text-gray-400">{m.requests} req</span>
                            <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{fmtCost(m.costUsd)}</span>
                          </div>
                        </div>
                        <div className="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                          <div className={`h-full ${providerColor} rounded-full`} style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <p className="text-sm text-gray-400 text-center py-4">No data yet — AI usage will appear here</p>
              )}
            </div>
            ...
            <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200 dark:border-gray-700/60 p-5 shadow-sm">
              ...
              {data?.byModule && data.byModule.length > 0 ? (
                ...
                <p className="text-sm text-gray-400 text-center py-4">No data yet — feature usage will appear here</p>
              )}
            </div>
          </div>
          ...
          {totalRequests === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">No AI usage yet</h3>
              <p className="text-xs text-gray-400 max-w-xs mx-auto">
                Cost data appears here after your first AI call. Try Business Intelligence or any AI feature to start tracking.
              </p>
            </div>
          )}
        </>
