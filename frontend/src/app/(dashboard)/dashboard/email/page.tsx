...
import { Plus, Mail, Send, Trash2, FileText, Sparkles, Loader2, X } from 'lucide-react';
...
function TemplateModal({ onClose }: { templates: any[]; onClose: () => void }) {
  ...
  async function generateWithAI() {
    const purpose = aiPurpose || form.name;
    if (!purpose) return toast.error('Enter a purpose or template name first');
    setAiLoading(true);
    try {
      const { data } = await api.post('/ai/email-template', { purpose, tone: 'professional', audience: 'customers' });
      const result = data.data;
      setForm(f => ({ ...f, subject: result.subject || f.subject, body: result.body || f.body }));
      toast.success('Email template generated by AI');
      setShowAI(false);
    } catch {
          ...
          {showAI ? (
            <div className="rounded-xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/30 p-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-indigo-700 dark:text-indigo-300">Generate with AI</p>
                <p className="text-xs text-indigo-500 mb-2">Describe what this email should do</p>
                <div className="flex gap-2">
                  <input value={aiPurpose} onChange={e => setAiPurpose(e.target.value)} placeholder="e.g. Welcome new customer, invoice reminder" className="flex-1 text-sm rounded-lg px-3 py-1.5 border border-indigo-200 dark:border-indigo-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white outline-none" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); generateWithAI(); } }} />
                  <button type="button" onClick={generateWithAI} disabled={aiLoading} className="px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50">{aiLoading ? 'Writing…' : 'Generate'}</button>
                  <button type="button" onClick={() => setShowAI(false)} className="text-indigo-400 hover:text-indigo-600 px-1"><X className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ) : (
            <button type="button" onClick={() => setShowAI(true)} className="flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
