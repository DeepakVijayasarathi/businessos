...
function ActivityModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal onClose={onClose} title="Log Activity" subtitle="Record a call, email, meeting, or note" icon={Calendar} iconColor="orange">
      <form onSubmit={e => { e.preventDefault(); mutation.mutate(form); }}>
        <div className="p-6 space-y-4">
          <TextField id="activity-title" label="Title" required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
          <div className="grid grid-cols-2 gap-4">
            ...
            <TextField id="activity-dueDate" label="Due Date" type="datetime-local" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} />
          </div>
          <TextAreaField id="activity-notes" label="Notes" rows={4} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
