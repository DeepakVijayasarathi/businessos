...
import { Plus, TrendingDown, Receipt, CheckCircle2, XCircle } from 'lucide-react';
...
export default function ExpensesPage() {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => api.post(`/finance/expenses/${id}/reject`),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <ExportButton endpoint="/finance/expenses/export" filename="expenses.csv" params={{ status }} />
              ...
              <tr key={exp.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-900 dark:text-white font-medium">{exp.title || exp.description}</span>
                  </div>
                </td>
                  ...
                  {exp.status === 'pending' && (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-green-50 dark:hover:bg-green-950/30 text-green-600 transition-colors disabled:opacity-50"
                        title="Approve"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 text-red-500 transition-colors disabled:opacity-50"
                        title="Reject"
                      >
...
function ExpenseModal({ onClose }: { onClose: () => void }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ title: '', amount: '', category: 'office', date: new Date().toISOString().split('T')[0], description: '' });
    ...
    <Modal onClose={onClose} title="Add Expense" subtitle="Log a new business expense" icon={TrendingDown} iconColor="red">
      <form onSubmit={e => { e.preventDefault(); mutation.mutate(form); }}>
        <div className="p-6 space-y-4">
          ...
          <TextAreaField id="expense-description" label="Notes" rows={2} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
        </div>
        <ModalFooter>
          ...
          <button type="submit" disabled={mutation.isPending || !form.title} className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 text-white text-sm font-medium disabled:opacity-50">{mutation.isPending ? 'Adding...' : 'Add Expense'}</button>
        </ModalFooter>
      </form>
    </Modal>
  );
